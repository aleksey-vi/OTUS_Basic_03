...
sudo sed -i 's/bind-address\t\t= 127.0.0.1/bind-address\t\t= 192.168.11.101/g' /etc/mysql/mysql.conf.d/mysqld.cnf

# [MASTER] Create user for replication
sudo mysql -e "create user repl@192.168.11.102 IDENTIFIED WITH caching_sha2_password BY '_password';"
sudo mysql -e "GRANT REPLICATION SLAVE ON *.* TO repl@192.168.11.102;"
